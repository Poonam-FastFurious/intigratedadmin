...
function ShareHolderdocs() {
  const [documents, setDocuments] = useState([]);
  const [loading, setLoading] = useState(true);
  const [deletingId, setDeletingId] = useState(null);
  const fetchDocuments = async () => {
    try {
      const res = await fetch(Baseurl + "shareholders");
      ...
      <div className="flex justify-between mb-10">
        <div className="page-title">
          <h3 className="text-2xl font-semibold">
            Shareholders Information Documents
          </h3>
          <p className="text-gray-600 text-md">
            All uploaded Shareholders Information documents
          </p>
        </div>
      </div>
                    ...
                    <td className="px-9 py-3 text-end">
                      <div className="flex items-center justify-end space-x-2">
                        <button className="w-10 h-10 bg-success text-white rounded-md hover:bg-green-600">
                          <Link to={`/shareholders/edit/${doc._id}`}>
                            <EditIcon size={18} />
                          </Link>
                        </button>
                        <button
...
export default ShareHolderdocs;
