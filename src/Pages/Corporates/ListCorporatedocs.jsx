...
function ListCorporatedocs() {
  const [documents, setDocuments] = useState([]);
  const [loading, setLoading] = useState(true);
  const [deletingId, setDeletingId] = useState(null);
  const fetchDocuments = async () => {
    try {
      const res = await fetch(Baseurl + "corporate-governance");
    ...
    <div className="body-content px-8 py-8 bg-slate-100">
      <div className="flex justify-between mb-10">
        <div className="page-title">
          <h3 className="text-2xl font-semibold">Corporate Governance Documents</h3>
          <p className="text-gray-600 text-md">
            All uploaded Corporate Governance documents
          </p>
        </div>
      </div>
                    ...
                    <td className="px-9 py-3 text-end">
                      <div className="flex items-center justify-end space-x-2">
                        <button className="w-10 h-10 bg-success text-white rounded-md hover:bg-green-600">
                          ...
                          <Link to={`/governance/edit/${doc._id}`} > <EditIcon size={18} /></Link>
                        </button>
                        <button
...
export default ListCorporatedocs;
